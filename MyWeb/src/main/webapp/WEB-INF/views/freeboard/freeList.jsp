<!-- 검색버튼, 페이지관련버튼 form태그 이용해서 한번에 보내기 
data-pagenum 여기에 대문자 있으면 안된다 . -->
...
<%@ taglib uri="http://java.sun.com/jsp/jstl/fmt" prefix="fmt" %> 
    ...
    <section>
        <div class="container-fluid">
		    ...
		    <!-- 페이지 관련 버튼(이전, 다음, 페이지번호)을 클릭했을 때 같이 숨겨서 보내줄 공통 값 선언 -->
	...
	<script>
		$(function() {
			...
			//그 버튼에 맞는 페이지 정보를 자바스크립트로 끌고와서 요청을 보내주겠습니다.
			
			//전파방식 (부모거에 걸기)
			$('#pagination').on('click','a',function(e) {	//a 태그에 클릭이벤트 발생한다면
				e.preventDefault(); //a태그의 고유기능 중지
				
				//현재 이벤트가 발생한 요소(버튼)의 data-pagenum의 값을 얻어서 변수에 저장.
				//const value = e.target.dataset.pageNum;	 //바닐라JS버전
				const value = $(this).data('pagenum');		 //jQuery버전
